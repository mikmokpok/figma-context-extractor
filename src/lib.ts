 ...
 * ```
 */
export function getImageNodeInfo(
...
function findImageAssets(nodes: any[], globalVars: any): any[] {
  ...
  function traverse(node: any) {
    const isImageAsset =
      node.type === "IMAGE-SVG" || hasImageFill(node, globalVars);
  ...
  imageAssets.forEach((asset, index) => {
    ...
    if (result && result.filePath) {
      ...
      if (useRelativePaths === false) {
        ...
        const basePath = useRelativePaths.endsWith("/")
          ? useRelativePaths
          : useRelativePaths + "/";
        ...
        if (normalizedFilePath.startsWith(normalizedBasePath)) {
          pathForMarkup =
            "./" + normalizedFilePath.substring(normalizedBasePath.length);
        } else {
